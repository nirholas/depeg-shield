# Integrating DepegShield

## From Solidity

```solidity
import {DepegShieldHook} from "depeg-shield/src/hooks/DepegShieldHook.sol";
...
hook.configure(key, DepegShieldHook.Config({ /* see the README */ }));
...
const address = hookAddress("depeg-shield", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "DepegShield"
...
More at https://depeg-shield.pages.dev.
