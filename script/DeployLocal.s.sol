...
contract DeployLocal is Script {
    ...
    function run() external {
        ...
        (address predicted, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER, FLAGS, type(DepegShieldHook).creationCode, abi.encode(IPoolManager(address(manager)))
        );
        hook = new DepegShieldHook{salt: salt}(IPoolManager(address(manager)));
    ...
    function _openPool() private {
        ...
        hook.configure(key, DepegShieldHook.Config({pegTick: 0, baseFee: 500, minFee: 100, maxSurcharge: 20_000, halfDeviationTicks: 200}));
